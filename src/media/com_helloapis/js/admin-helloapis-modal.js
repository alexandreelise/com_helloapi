...
(function () {
  ...
  /**
    ...
    * View element calls jSelectHelloapi when a helloapi is clicked
    * jSelectHelloapi creates the link tag, sends it to the editor,
    * and closes the select frame.
    */

  window.jSelectHelloapi = function (id, title, catid, object, link, lang) {
    var hreflang = '';

    if (!Joomla.getOptions('xtd-helloapis')) {
      // Something went wrong
      window.parent.Joomla.Modal.getCurrent().close();
      return false;
    }

    var _Joomla$getOptions = Joomla.getOptions('xtd-helloapis'),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    for (var i = 0, l = elements.length; l > i; i += 1) {
      ...
      elements[i].addEventListener('click', function (event) {
        ...
        if (functionName === 'jSelectHelloapi') {
          // Used in xtd_helloapis
